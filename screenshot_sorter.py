...
for i in file_list:
    for b in os.listdir(path_main+"\\content\\cars"): # find the name of the car
        ...
        if i[11:11+car_string_length] == b.lower(): # 11 = bypass Screenshot_
            ...
            if not os.path.exists(path_screens+f"\\{b}"): # create a new folder if there isn't already one
                os.mkdir(path_screens+f"\\{b}")
            shutil.move(path_screens+f"\\{i}", path_screens+f"\\{b}\\{i}")
